...
export async function psqlOneShot(
  ...
  const inner = `docker exec -i supabase-db-d12fsiflbur48p3s2f8dz2j2 psql -U ${user} -d postgres -P pager=off -c ${shellQuote(sql)}`;
...
export async function psqlPiped(
  ...
  const remoteCmd = `docker exec -i supabase-db-d12fsiflbur48p3s2f8dz2j2 psql -U ${user} -d postgres -P pager=off -v ON_ERROR_STOP=1`;
  const proc = Bun.spawn({
